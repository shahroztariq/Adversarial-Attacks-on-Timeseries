...
class Plotter:
    ...
    def plot_feature(self, feature, plot_train=False, plot_errors=True, plot_feature_anom=False, start=None, end=None):
        ...
        for nr, data_copy in enumerate(plot_data):
            ...
            fig.write_html(os.path.join(self.result_path, "plot_feature.html"))
            fig.write_image(os.path.join(self.result_path, "plot_feature.pdf"))
            # py.offline.plot(fig)
            ...
            if plot_errors:
                ...
                e_fig.write_html(os.path.join(self.result_path, "plot_feature_plot_errors.html"))
                e_fig.write_image(os.path.join(self.result_path, "plot_feature_plot_errors.pdf"))
    ...
    def plot_all_features(self, start=None, end=None, type="test"):
        ...
        colors = ["teal", "darkgoldenrod", "royalblue", "r"] * (num_cols // 4) + ["b", "g"]
        axes=data_copy.plot(subplots=True, figsize=(20, num_cols), ylim=(0, 1.5), style=colors, legend=True)
        ...
        # x1.legend("HHZ 1", loc="upper right")
        # ax2.legend("HHN", loc="upper right")
        # ax3.legend("HHE", loc="upper right")
        plt.savefig(os.path.join(self.result_path, "plot_all_features.pdf"), format='pdf')
        # axes.legend(cols, loc='lower center', bbox_to_anchor=(0.5, -0.20, 0.0, 0.0), ncol=4, fancybox=True)
        plt.show()
    ...
    def plot_anomaly_segments(self, type="test", num_aligned_segments=None, show_boring_series=False):
        ...
        fig.write_html(os.path.join(self.result_path, "plot_anomaly_segments.html"))
        fig.write_image(os.path.join(self.result_path, "plot_anomaly_segments.pdf"))
        # py.offline.plot(fig)
    ...
    def plot_global_predictions(self, type="test"):
        ...
        axs[0].plot(data_copy[f"A_Score_Global"][18000:38000], c="r", label="Anomaly Scores")

        axs[0].plot(data_copy["Thresh_Global"][18000:38000], linestyle="dashed", c="black", label="Threshold")
        axs[1].plot(data_copy["A_Pred_Global"][18000:38000], label="Predicted Anomalies", c="darkgoldenrod")
        if self.labels_available and type == "test":
            axs[2].plot(
                data_copy["A_True_Global"][18000:38000],
                label="Ground Truth",c="royalblue"
            )
        # axs[0].set_ylim([0, 5 * np.mean(data_copy["Thresh_Global"].values)])
        fig.legend(prop={"size": 20},loc='lower center',ncol=4,fancybox=True)
        plt.savefig(os.path.join(self.result_path, "plot_global_predictions.pdf"), format='pdf')
        plt.savefig(os.path.join(self.result_path, "plot_global_predictions.svg"), format='svg')
        plt.show()
    ...
    def plotly_global_predictions(self, type="test"):
        ...
        # fig.update_xaxes(t)
        fig.write_html(os.path.join(self.result_path,"plotly_global_predictions.html"))
        fig.write_image(os.path.join(self.result_path,"plotly_global_predictions.pdf"))
        # py.offline.plot(fig)
