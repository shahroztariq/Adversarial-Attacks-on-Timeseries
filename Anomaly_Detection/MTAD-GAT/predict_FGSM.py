...
if __name__ == "__main__":
    ...
    load (model_adv, f"{model_path}/model.pt", device=device)
    model.to(device)
    model_adv.to(device)
    ...
    # x_FGSM_linf = fast_gradient_method(model, x_test, FLAGS.eps, np.inf,y=label)
    # print("x_FGSM_linf.shape", x_FGSM_linf.shape)
    predictor = Predictor(model,model_adv, window_size, n_features, prediction_args, summary_file_name=summary_file_name)
    predictor.predict_anomalies_FGSM(x_train, x_test, label,
